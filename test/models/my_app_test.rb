...
class MyAppTest < ActiveSupport::TestCase
  ...
  test "request for something other than name or id for by parameter results in error" do 
    assert_equal(MyApp.search(by: "idd", start: 10), {:success=>false, :message=>"Only id and name are permitted for by"})
  end
  ...
  #Even if the "start" identifier is included, the "end" identifier can be omitted, in whcih case the program queries with no ending bound, but will still return results for default "max" page size (50)
